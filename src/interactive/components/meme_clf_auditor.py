...
from src.generated.site_tables import (ImgflipTemplates, RedditMemes,
                                       TemplatePredictions)
from src.interactive.components.base import IPyProtocol
from src.interactive.components.meme_clf_audit_stats import display_audit_stats
from src.interactive.inputs.get_version import get_meme_version
...
class IPyMemeClfAuditor(IPyProtocol):
    ...
    def entry_point(cls):
        ...
        with site_session_maker() as session:
            cls.template_name_urls: dict[str,str] = {name: url for name, url in session.execute(select(ImgflipTemplates.name, ImgflipTemplates.url))}
        ...
        for tensors, ids in MemeClfPreditingDataset.data_loader(Entity.RedditMeme,
            ...
            for tensor, id, name in zip(tensors, ids, static_data.get_names(int_names)):
